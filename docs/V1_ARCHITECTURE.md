...
Runtime observation 与 Provider Health 共用 provider readiness 词汇：provider mode 只能是 `local_fallback`、`fake_provider` 或 `live_provider`；状态只能是 `healthy`、`degraded`、`unavailable` 或 `not_configured`。标准 provider gap 与 health 语义保持一致：`provider_configuration_missing` 对应 `not_configured`，`provider_failure` / `provider_timeout` / `provider_auth_error` 对应 `unavailable`，`provider_empty_result` / `provider_schema_mismatch` / `provider_budget_exceeded` 对应 `degraded`。这些 gap 只能进入 trace / dashboard 作为只读可观测证据，不能变成 mastery、推荐排序或可信 RAG citation。
