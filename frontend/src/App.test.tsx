...
describe("学习驾驶舱", () => {
  ...
  it("可以加载建议、提交推荐题答案，并展示 trace 与证据", async () => {
    ...
    expect(screen.getAllByText(/local_fallback · gaps 0/).length).toBeGreaterThanOrEqual(3);
    expect(screen.getAllByText(/正常/).length).toBeGreaterThanOrEqual(3);
    expect(screen.getAllByText(/fallback/).length).toBeGreaterThanOrEqual(3);
    ...
    expect(screen.getByText("KT facts are authoritative.")).toBeInTheDocument();
    expect(screen.getByText("RAG can support explanation, not overwrite prediction facts.")).toBeInTheDocument();
